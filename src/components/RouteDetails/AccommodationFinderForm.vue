    ...
    <div class="finder-content">
      <div v-if="!searching && !accommodations.length">
        <form @submit.prevent="findAccommodation">
          <div class="form-group">
            ...
            <select id="segment-select" v-model="form.segmentId" required>
              <option value="" disabled>Select a route segment</option>
              <option v-for="segment in longSegments" :key="segment.id" :value="segment.id">{{ getSegmentName(segment) }} ({{ formatDuration(segment.duration) }})</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="price-slider-container">
              <input type="range" id="max-price" v-model.number="form.maxPrice" min="50" max="500" step="10" class="price-slider" />
          ...
          <div class="form-group">
            ...
            <div class="rating-select">
              <div v-for="rating in [1, 2, 3, 4, 5]" :key="rating" class="star-rating" :class="{ active: form.minRating >= rating }" @click="form.minRating = rating">
          ...
          <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary" :disabled="!form.segmentId">Find Accommodations</button>
          </div>
        </form>
      </div>
      ...
      <div v-else>
        ...
        <div class="accommodations-list">
          <div v-for="(hotel, index) in accommodations" :key="index" class="hotel-card">
            <div class="hotel-image">
              <img :src="hotel.image_url || 'https://via.placeholder.com/150x100?text=Hotel'" :alt="hotel.name" />
            </div>
            <div class="hotel-details">
              <h5>{{ hotel.name }}</h5>
              <div class="hotel-rating">
                <i v-for="star in 5" :key="star" class="fas" :class="star <= Math.round(hotel.rating) ? 'fa-star' : 'fa-star-o'"></i>
                <span class="rating-text">{{ hotel.rating.toFixed(1) }}</span>
              </div>
              <p class="hotel-price">${{ hotel.price }} per night</p>
              <p v-if="hotel.address" class="hotel-address"><i class="fas fa-map-marker-alt"></i> {{ hotel.address }}</p>
            </div>
          </div>
        </div>
...
<!-- src/components/RouteDetails/AccommodationFinderForm.Vue -->
